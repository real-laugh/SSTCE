...
def CalfitFJFP(p_chrom, m_chrom, FJ, f_index, N, H, TM, time):
    ...
    for i in range(SH):
        ...
        MS[mm[i]].append(t1*10 + t2)
    # initial total workload and total idle time
    TWL = 0
    TIT = 0
    operationMFT = []
    for i in range(5):
        temp = np.zeros(shape=(5, len(MS[i])))
        operationMFT.append(temp)
    unique_elements, _ = np.unique(p_chrom, return_counts=True)
    different_count = len(unique_elements)
    operationJFT = np.zeros(shape=(different_count, 5))
    # start decoding
    for i in range(SH):
    ...
    return fitness[0], fitness[1], finish, unique_elements, mt
...
def CalfitDHFJFP(p_chrom, m_chrom, f_chrom, N, H, SH, F, TM, time):
    ...
    Finish = np.zeros(shape=(N, int(max(H))))
    MT = []
    for f in range(F):
        sub_f_fit[f][0], sub_f_fit[f][1], finish, unique_elements, mt = CalfitFJFP(P[f], m_chrom, FJ[f], f, N, H, TM, time)
        MT.append(mt)
        for j in range(len(unique_elements)):
            Finish[unique_elements[j]] = finish[unique_elements[j]]
    fit1 = sub_f_fit[0][0]
    fit3 = 0
    fit2 = 0
    sub_f_time = []
    for f in range(F):
        ...
        sub_f_time.append(sub_f_fit[f][0])
        if fit1 < sub_f_fit[f][0]:
            fit1 = sub_f_fit[f][0]
            fit3 = f
    return fit1, fit2, fit3, sub_f_time, Finish, MT
