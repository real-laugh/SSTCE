...
def CalculateLastOperation(s1, s2, mm, digraph, last, VELTime, Vflag, time, f_index):
    ...
    if digraph[Index][2] != 0:
        ...
        if Vflag[zlast][0] == 0:
            VELTime[zlast][0], VELTime, Vflag = CalculateLastOperation(s1, s2, mm, digraph, zlast, VELTime, Vflag, time,
                                                                       f_index)
            Vflag[zlast][0] = 1
...
def CalculateNextOperation(s1, s2, mm, digraph, next, VELTime, Vflag, LastNode, time, f_index):
    ...
    if digraph[Index][0] == 0:
        ...
        if Vflag[znext][1] == 0:
            VELTime[znext][1], VELTime, Vflag = CalculateNextOperation(s1, s2, mm, digraph, znext, VELTime, Vflag,
                                                                       LastNode, time, f_index)
            Vflag[znext][1] = 1
...
def FindCriticalPathDHFJSP(p_chrom, m_chrom, FJ, f_index, N, H, time, TM):
    ...
    for i in range(SH):
        ...
        if digraph[Index][3] != 0:
            ...
            if Vflag[last][0] == 0:
                VELTime[last][0], VELTime, Vflag = CalculateLastOperation(s1, s2, mm, digraph, last, VELTime, Vflag,
                                                                          time, f_index)
                Vflag[last][0] = 1
    ...
    for i in range(SH - 1, -1, -1):
        ...
        if digraph[Index][0] == 0:
            if digraph[Index][1] == 0:
                ...
                if Vflag[next][1] == 0:
                    VELTime[next][1], VELTime, Vflag = CalculateNextOperation(s1, s2, mm, digraph, next, VELTime, Vflag,
                                                                              LastNode, time, f_index);
                    Vflag[next][1] = 1
